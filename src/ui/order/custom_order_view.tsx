  ...
  renderTotal(currency: Currency) {
    return <React.Fragment>
      <Row flex={1} style={{ alignItems: 'center' }}>
        <Column flex={11}></Column>
        <Column flex={3} className="patterns-item-document-header-column">CENA MATERIÁL</Column>
        <Column flex={3} className="patterns-item-document-total">
          { PatternApp.format.price(parseFloat(this.props.document.itemsPrice().getValue()) / 1.2, currency) }
        </Column>
        <Column flex={4} className="patterns-item-document-total">
          { PatternApp.format.price(parseFloat(this.props.document.itemsPrice().getValue()), currency) }
        </Column>
      </Row>
      <Row flex={1} style={{ alignItems: 'center' }}>
        <Column flex={11}></Column>
        <Column flex={3} className="patterns-item-document-header-column">CENA PRÁCA</Column>
        <Column flex={3} className="patterns-item-document-total">
          { PatternApp.format.price(parseFloat(this.props.document.workPrice().getValue()) / 1.2, currency) }
        </Column>
        <Column flex={4} className="patterns-item-document-total">
          { PatternApp.format.price(parseFloat(this.props.document.workPrice().getValue()), currency) }
        </Column>
      </Row>
      <Row flex={1} style={{ alignItems: 'center' }}>
        <Column flex={11}></Column>
        <Column flex={3} className="patterns-item-document-header-column">CENA SPOLU</Column>
        <Column flex={3} className="patterns-item-document-total">
          { PatternApp.format.price(this.props.document.itemsTotal, currency) }
        </Column>
        <Column flex={4} className="patterns-item-document-total">
          { PatternApp.format.price(this.props.document.itemsTotalWithVat, currency) }
        </Column>
      </Row>
    </React.Fragment>
  }
      ...
      <Page>
        <Header>
          ...
          <Column flex={4} style={{ flex: 4 }}>
            <HeaderCode>
              <div className="patterns-item_document-header-name">
                { this.props.document.exists ? this.props.document.code : 'Nová objednávka' }
              </div>
            </HeaderCode>
        ...
        <Row className="patterns-item-document-dates">
          ...
          <Row flex={2} className="patterns-item-document-dates-column">
            <Column style={{ marginLeft: 5, fontWeight: 'bold' }}>
              <InputGroup
                style={{ marginTop: 4 }}
                value={this.props.document.issuedBy}
                onChange={evt => {
                  this.props.onChange('issuedBy', evt.currentTarget.value)
                }}
              />
            ...
            <Button
              ...
              onClick={() => {
                ...
                order.items.push(new DocumentItem({
                  index: this.props.document.items.length,
                  quantity: 1,
                  vat: 20
                }))
