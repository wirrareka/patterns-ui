...
export default function OrderPrinter({ document }: Props): ReactElement {
  ...
  const renderHeader = (page: number) => {
    return <Row key={`header-${page}`} style={{ marginLeft: '12mm', marginRight: '12mm', marginTop: '12mm', marginBottom: '2mm', paddingBottom: 4, borderBottomWidth: 2, borderColor: Colors.LIGHT_GRAY3, }}>
      <Column style={{ flex: 6 }}>
  ...
  const getPages = () => {
    let rows = document.items.map(item => item.code.length > 0 ? 2 : 1).reduce((acc, next) => acc += next, 0)
    let perPager = (rows - 36) / 63
